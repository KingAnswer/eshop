...
import com.qingshixun.project.eshop.module.order.util.PagUtil;
...
public class OrderServiceImpl extends BaseService {
     ...
     * @param userpageNo 
     */
    public PagUtil<OrderDTO> getOrdersByMember(Long memberId, int userpageNo) {
    	PagUtil<OrderDTO> pagutil = new PagUtil<OrderDTO>();
    	int start=(userpageNo-1)*10;
    	int count=10;
    	pagutil.setPageNo(userpageNo);
    	
        List<OrderDTO> orders = orderDao.getOrdersByMember(memberId,start,count);
		...
		//页码需要引入小数进行运算，只要不能整除就加一
        int x = orderDao.getallOrder().size()/10+1;
        
		pagutil.setPageTotal(x);
		//获取当前页的数据	
		pagutil.setData(orders);
        return pagutil;
    }
