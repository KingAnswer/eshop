...
public class OrderController extends BaseController {
    ...
    public String orderList(Model model,@RequestParam(value="userpageNo", defaultValue="1")int userpageNo) {
        ...
        // 传递我的订单数据
        model.addAttribute("orders", orderService.getOrdersByMember(member.getId(),userpageNo));
        // 传递登录会员数据
        model.addAttribute("member", member);
        
   	 	//往前端传数据
  		//userpojo 查询的数据信息
  		model.addAttribute("userpageNo", userpageNo);
     ...
     * @return
     */
    @RequestMapping(value ="/deta/{orderId}", method = RequestMethod.GET)
     ...
     * 
     */
    @RequestMapping(value="/detas/{orderId}", method = RequestMethod.GET)
    public String delect(Model model , @PathVariable Long orderId,@RequestParam(value="userpageNo", defaultValue="1")int userpageNo) {
    	System.out.println(orderId);
    	MemberDTO member = this.getCurrentUser();
    	orderItemService.delect(orderId);
    	 // 传递商品分类数据
        model.addAttribute("productCategories", productCategoryService.getProductCategories());
        // 传递我的订单数据
        model.addAttribute("orders", orderService.getOrdersByMember(member.getId(), userpageNo));
        // 传递登录会员数据
        model.addAttribute("member", member);
    	return "redirect:/front/order/list";
    }
     ...
     * @return
     */
    @RequestMapping(value ="/payments/{params}/{receiverId}")
    @ResponseBody
    public String alipay( @PathVariable String params, @PathVariable Long receiverId) throws AlipayApiException{
      MemberDTO member = this.getCurrentUser();
      Long orderId = orderService.commitOrder(params, member, receiverId, getSession());
      OrderDTO order = orderService.getOrder(orderId);
      String TotalMone = String.valueOf(order.getTotalAmount());
      String id = String.valueOf(order.getId());
      AlipayBean alipayBean=new AlipayBean();
      alipayBean.setBody(order.getCreateTime());
      alipayBean.setTotal_amount(TotalMone);
      alipayBean.setOut_trade_no(id);
      alipayBean.setSubject(order.getOrderNum());
